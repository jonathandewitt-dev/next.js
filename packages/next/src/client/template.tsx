...
let dsdSupported: boolean | null = null;
function checkDSDSupport(): boolean {
  if (dsdSupported !== null) return dsdSupported;
  ...
  dsdSupported = !!fragment.querySelector('div')?.shadowRoot;
  return dsdSupported;
}
...
>((props, forwardedRef) => {
  ...
  const shadowOptsRef = React.useRef({
    delegatesFocus,
    mode: shadowrootmode,
  });
  ...
  React.useEffect(() => {
    ...
    if (typeof forwardedRef === 'function') {
      ...
      forwardedRef.current = templateRef.current;
    }
    ...
    if (shadowrootmode && !shadowRootRef.current) {
      const existingShadowRoot = templateRef.current.parentNode.shadowRoot;
      if (existingShadowRoot) {
        shadowRootRef.current = existingShadowRoot;
      } else {
        try {
          const parent = templateRef.current.parentNode;
